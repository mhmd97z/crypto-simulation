...
contract EBookLibrary is CustomERC20{
    ...
    public{
	    ...
	    rentedBooks[bookId] = rentInfo(admin, now, true);
    }
    ...
    payable{
        //TODO
    	if(balanceOf(msg.sender) >= premiumFee){
    ...
    require(balanceOf(msg.sender) >= books[bookId].accessFee);
        ...
        if(rentedBooks[bookId].available){                                  // available 
            ...
            if(accoutTypes[msg.sender] != AccountType.premium){
                accoutTypes[msg.sender] = AccountType.regular;
            }
            
        }
        else{                                                               // not available
        
            if(now > rentedBooks[bookId].startTime + accessDuration){       //  finished access duration   
            ...
            else{                                                               // remaining access duration
                
                if(accoutTypes[msg.sender] == AccountType.premium){             // premium
                    
                    if(accoutTypes[rentedBooks[bookId].renter] == AccountType.regular) {               // regular former renter
                        
                        transfer(rentedBooks[bookId].renter, books[bookId].accessFee);
                        rentedBooks[bookId].renter = msg.sender;
                        rentedBooks[bookId].startTime = now;
                        rentedBooks[bookId].available = false;
                    
                    }
                    else{                                                                               // premium former renter
                        revert();
                    }
